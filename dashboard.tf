...
resource "grafana_dashboard" "payments_api_gateway" {
  count  = var.enable_grafana ? 1 : 0
  folder = grafana_folder.orbitpay[0].uid

  config_json = jsonencode({
    title   = "Payments API Gateway — SRE Overview"
    uid     = "payments-api-gateway"
    tags    = ["orbitpay", "sre", "payments"]
    refresh = "30s"
    ...
    panels = [
      ...
      },
      {
        ...
        targets = [{
          refId      = "A"
          datasource = { type = "prometheus", uid = local.prom_uid }
          expr       = "sum(rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\"}[2m]))"
        }]
        options = {
      ...
      },
      {
        ...
        targets = [{
          refId      = "A"
          datasource = { type = "prometheus", uid = local.prom_uid }
          expr       = "sum(rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\",http_status_code=~\"5..\"}[2m])) / sum(rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\"}[2m])) * 100 or vector(0)"
        }]
        options = {
          reduceOptions = { calcs = ["lastNotNull"] }
          orientation   = "auto"
      ...
      },
      {
        ...
        targets = [{
          refId      = "A"
          datasource = { type = "prometheus", uid = local.prom_uid }
          expr       = "histogram_quantile(0.99, rate(http_server_request_duration_seconds_bucket{service_name=\"payments-api-gateway\"}[2m]))"
        }]
        options = {
          reduceOptions = { calcs = ["lastNotNull"] }
          orientation   = "auto"
      ...
      },
      {
        ...
        targets = [{
          refId      = "A"
          datasource = { type = "prometheus", uid = local.prom_uid }
          expr       = "nodejs_heap_size_used_bytes{service_name=\"payments-api-gateway\"} / nodejs_heap_size_total_bytes{service_name=\"payments-api-gateway\"} * 100"
        }]
        options = {
          reduceOptions = { calcs = ["lastNotNull"] }
          orientation   = "auto"
        ...
        targets = [
          {
            refId        = "A"
            datasource   = { type = "prometheus", uid = local.prom_uid }
            expr         = "sum by (http_status_code) (rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\",http_status_code=~\"2..\"}[2m]))"
            legendFormat = "2xx"
          },
          {
            refId        = "B"
            datasource   = { type = "prometheus", uid = local.prom_uid }
            expr         = "sum by (http_status_code) (rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\",http_status_code=~\"4..\"}[2m]))"
            legendFormat = "4xx"
          },
          {
            refId        = "C"
            datasource   = { type = "prometheus", uid = local.prom_uid }
            expr         = "sum by (http_status_code) (rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\",http_status_code=~\"5..\"}[2m]))"
            legendFormat = "5xx"
          },
        ]
        fieldConfig = {
      ...
      },
      {
        ...
        targets = [{
          refId        = "A"
          datasource   = { type = "prometheus", uid = local.prom_uid }
          expr         = "sum(rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\",http_status_code=~\"5..\"}[2m])) or sum(rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\"}[2m]) * 0)"
      ...
      # ── Row 3: Process Transfer Endpoint ────────────────────────────────
      {
        id        = 20
        type      = "row"
        title     = "Process Transfer Endpoint"
      ...
      },
      {
        id         = 21
        type       = "timeseries"
        title      = "Process Transfer Error Rate"
        gridPos    = { x = 0, y = 15, w = 12, h = 8 }
        datasource = { type = "prometheus", uid = local.prom_uid }
        targets = [{
          refId        = "A"
          datasource   = { type = "prometheus", uid = local.prom_uid }
          expr         = "sum(rate(http_server_request_duration_seconds_count{service_name=\"payments-api-gateway\",http_route=\"/api/process-transfer\",http_status_code=~\"[45]..\"}[2m])) or vector(0)"
      ...
      },
      {
        id         = 22
        type       = "timeseries"
        title      = "Process Transfer p99 Latency"
        gridPos    = { x = 12, y = 15, w = 12, h = 8 }
        datasource = { type = "prometheus", uid = local.prom_uid }
        targets = [{
          refId        = "A"
          datasource   = { type = "prometheus", uid = local.prom_uid }
          expr         = "histogram_quantile(0.99, rate(http_server_request_duration_seconds_bucket{service_name=\"payments-api-gateway\",http_route=\"/api/process-transfer\"}[2m]))"
      ...
      },
      {
        ...
        targets = [{
          refId        = "A"
          datasource   = { type = "prometheus", uid = local.prom_uid }
          expr         = "nodejs_eventloop_lag_seconds{service_name=\"payments-api-gateway\"}"
        ...
        targets = [
          {
            refId        = "A"
            datasource   = { type = "prometheus", uid = local.prom_uid }
            expr         = "nodejs_heap_size_used_bytes{service_name=\"payments-api-gateway\"}"
            legendFormat = "used"
          },
          {
            refId        = "B"
            datasource   = { type = "prometheus", uid = local.prom_uid }
            expr         = "nodejs_heap_size_total_bytes{service_name=\"payments-api-gateway\"}"
            legendFormat = "total"
          },
        ]
        fieldConfig = {
